...
`0.1.0-alpha.9` 不再提供独立媒体面板。会话输入栏中只有一个位于 Harness 原有附件按钮旁边的彩色 `+`；用户选择文件后，附件成为主草稿中的原生引用 chip，问题继续写在主输入框，并随 Harness 原生发送、Enter、排队或插话动作一起提交。
...
预算开启且单块已被接纳后，Agent `tool-result` 图片／视频的 resolver、声明长度完整性或 wire 编码失败会只省略该块、退还它预占的预算并附加无本机路径的诊断；同一工具文本及后续可用媒体仍继续。用户取消、显式模态禁用、无效预算，以及逐块处理前的请求级 Node 字符串可表示性／实时内存门禁仍按原错误中止。后两项是防止进程资源耗尽的事实边界，不能包装成“任何媒体失败都保证不断链”。
