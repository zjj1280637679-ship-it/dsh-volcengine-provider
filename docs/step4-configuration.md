...
当前实现中，`raw` 直接使用用户提供的完整 JSON 对象，不预先读取 Harness 消息附件或进行常规消息序列化；`merge` 与 `patch` 在常规序列化后应用。用户在 raw 模式自行负责完整 wire 请求体。卡片校验 JSON 对象，不自动从供应商反馈或运行时结果推导请求参数。
...
第四步完成时尚无真实 API 密钥，因此该阶段没有发送方舟请求。后续 Coding Plan 的文本与媒体冒烟结果分别见 [文本实测](live-coding-plan-2026-09-05.md) 和 [媒体实测](live-coding-plan-media-2026-09-05.md)，完整 Web 安装闭环见[完整宿主闭环报告](harness-web-loop-2026-09-06.md)；这些结果只覆盖当次账号、通道、模型别名和样本。当前保留 `private: true`，不进行 npm 发布；后续版本通过 GitHub 预发布 Release 交付。
