...
const packageRoot = process.env.DSH_RELEASE_PACKAGE_ROOT
if (!packageRoot) throw new Error('DSH_RELEASE_PACKAGE_ROOT must point to the extracted, verified release package')
const { VolcengineChatAdapter, getDefaultRoute, buildMediaCommandContent } = await import(pathToFileURL(resolve(packageRoot, 'dist/index.js')).href)
...
report.releaseGate = {
  requestBudget: report.requests.length === 7,
  text: report.cases.filter(x => x.modality === 'text').length === 2 && report.cases.filter(x => x.modality === 'text').every(x => x.completed && x.semanticMatch),
  mediaBytes: report.cases.filter(x => x.modality !== 'text').length === 5 && report.cases.filter(x => x.modality !== 'text').every(x => x.bytesPreserved),
  imageAndVideoCompletion: report.cases.filter(x => x.modality === 'image' || x.modality === 'video').length === 4 && report.cases.filter(x => x.modality === 'image' || x.modality === 'video').every(x => x.completed),
  audioAcceptedOrExplicitlyRejected: report.cases.some((x, index) => x.modality === 'audio' && (x.completed || (x.failure?.code === 'INVALID_REQUEST' && report.requests[index]?.status === 400))),
}
report.ok = Object.values(report.releaseGate).every(Boolean)
const safeReport = JSON.stringify(report, (_name,value) => typeof value === 'string' ? scrub(value) : value, 2)
await writeFile(reportPath, safeReport + '\n')
